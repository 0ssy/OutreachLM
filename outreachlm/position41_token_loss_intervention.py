...
from outreachlm.position41_decision_analysis import (
    systematic_position_41_analysis,
)
...
def position_token_loss(logits, targets, position_index, loss_function):
    if position_index < 0:
        raise ValueError("position_index must be non-negative.")
    if position_index >= logits.shape[1]:
        raise ValueError("position_index exceeds sequence length in logits.")

    return loss_function(
        logits[:, position_index, :],
        targets[:, position_index],
    )
...
def run_intervention(
    ...
    for step in range(1, steps + 1):
        ...
        position_losses.append(float(pos_loss.item()))
        ...
        if step % max(steps // 4, 1) == 0 or step == steps:
            ...
            checkpoint_position41 = systematic_position_41_analysis(
                model=model,
                tokenizer=tokenizer,
            ...
            checkpoints.append(
                {
                    ...
                    "position41": checkpoint_position41,
                }
            )
            model.train()
    ...
    final_position41 = systematic_position_41_analysis(
        model=model,
        tokenizer=tokenizer,
        validation_token_ids=validation_token_ids,
        prompt_length=40,
    ...
    return {
        ...
        "loss_summary": {
            ...
            "first_position_loss": float(position_losses[0]) if position_losses else None,
            "last_position_loss": float(position_losses[-1]) if position_losses else None,
        },
        "checkpoint_metrics": checkpoints,
        "final_metrics": final_metrics,
        "final_position41": final_position41,
    }


def render_report(results):
    lines = []
    lines.append("OUTREACHLM POSITION-41 TOKEN-LOSS INTERVENTION")
    lines.append("=" * 80)
    ...
    lines.append(
        ...
        "recovery_start={rs} recovery_weight={rw} position_index={pi} position_weight={pw}".format(
            steps=cfg["steps"],
            ...
            pi=cfg["position_index"],
            pw=cfg["position_loss_weight"],
        )
    )
    lines.append("")
    ...
    for condition in ["v2_w2_checkpoint", "v2_after_position41_intervention"]:
    ...
    lines.append("")
    before = results["position41_before"]
    after = results["run"]["final_position41"]
    lines.append("POSITION-41 SYSTEMATIC STATS")
    lines.append("-" * 80)
    lines.append(
        "before teacher/free match: "
        f"{before['teacher_match_rate_at_pos41']:.6f} / {before['free_match_rate_at_pos41']:.6f}"
    )
    lines.append(
        "after  teacher/free match: "
        f"{after['teacher_match_rate_at_pos41']:.6f} / {after['free_match_rate_at_pos41']:.6f}"
    )
    lines.append(
        "before most-common wrong pred: "
        f"{before['most_common_wrong_pred_token']['token']!r} "
        f"frac={before['most_common_wrong_pred_token']['fraction_of_mismatches']:.6f}"
    )
    lines.append(
        "after  most-common wrong pred: "
        f"{after['most_common_wrong_pred_token']['token']!r} "
        f"frac={after['most_common_wrong_pred_token']['fraction_of_mismatches']:.6f}"
    )
    return "\n".join(lines)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Surgical position-41 token-loss intervention from current w=2.0 leader checkpoint."
        ),
    )
    parser.add_argument(
        "--resume-artifact",
    ...
    parser.add_argument("--position-index", type=int, default=40)
...
def main():
    ...
    baseline_position41 = systematic_position_41_analysis(
        model=model,
        tokenizer=tokenizer,
        validation_token_ids=validation_token_ids,
        prompt_length=40,
    ...
    summary_table = {
        "v2_w2_checkpoint": metric_row(baseline_metrics),
        "v2_after_position41_intervention": metric_row(run["final_metrics"]),
    }

    args.output_dir.mkdir(parents=True, exist_ok=True)
    stamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    artifact_path = args.output_dir / f"v2-position41-intervention-{stamp}.pt"
    artifact = build_model_artifact(
        ...
        training_config={
            ...
            "objective": "balanced_ce_ls_recovery_plus_position41_loss",
        },
    )
    ...
    results = {
        ...
        "v2_w2_checkpoint": baseline_metrics,
        "position41_before": baseline_position41,
        "run": run,
        "summary_table": summary_table,
        "artifact_path": str(artifact_path.resolve()),
    }

    json_path = args.output_dir / f"v2-position41-intervention-{stamp}.json"
    txt_path = args.output_dir / f"v2-position41-intervention-{stamp}.txt"
